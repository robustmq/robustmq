...
# 4. Uploads tar.gz files to the GitHub release
#
# Usage:
#   ./release.sh [OPTIONS]
#   
# Examples:
#   ./release.sh                        # Create release for version in Cargo.toml
...
${BOLD}OPTIONS:${NC}
    ...
    -p, --platform PLATFORM    Target platform to build (default: all)
...
${BOLD}EXAMPLES:${NC}
    # Create release for version in Cargo.toml
    $0
...
upload_packages() {
    ...
    for checksum in "$build_dir"/robustmq-$version-*.tar.gz.sha256; do
        if [ -f "$checksum" ]; then
            if upload_release_asset "$release_id" "$checksum"; then
                ((uploaded_count++))
            else
                ((failed_count++))
            fi
