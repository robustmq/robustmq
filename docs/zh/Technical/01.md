...
1. **Metadata State Machine**：存储集群相关元数据。数据存储在Column Family: Metadata.
2. **MQTT State Machine**：存储 MQTT 相关元数据。数据存储在Column Family: Data。
   - Session
   - Last Will
   - Retain Message
3. **Offset State Machine**：存储消费偏移量数据。数据存储在Column Family: Data。
