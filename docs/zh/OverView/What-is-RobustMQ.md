...
基于 NATS 协议扩展的 `$mq9.AI.API.*` Subject 空间，提供 Agent 注册、发现、调用、负载均衡的原生通信能力。不依赖 LangChain 等外部框架，任何 NATS 客户端（Go/Rust/Python/Java）即可接入，零学习成本。

```
Agent 注册  → PUB $mq9.AI.API.AGENT.REGISTER
Agent 发现  → PUB $mq9.AI.API.AGENT.DISCOVER
Agent 调用  → PUB $mq9.AI.API.AGENT.INVOKE.{name}
...
- 🤖 **AI Agent 通信**：基于 NATS 的 `$mq9.AI.API.*` 扩展，原生支持 Agent 注册、发现、调用和编排
...
| $mq9.AI.API.* Agent 通信 | 🔬 Demo 验证完成，开发中 |
