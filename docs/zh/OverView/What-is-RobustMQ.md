...
mq9 是 RobustMQ 专为 AI Agent 设计的第五种原生协议。每个 Agent 拥有一个邮箱（mailbox），消息写入后持久化存储；接收方上线即推送，发送方和接收方不需要同时在线。支持三级优先级（critical / urgent / normal）、TTL 自动清理、公开邮箱服务发现。

```
创建邮箱  → nats req  '$mq9.AI.MAILBOX.CREATE'           '{"ttl":3600}'
发送消息  → nats pub  '$mq9.AI.MAILBOX.MSG.{mail_address}'    '{...}'
紧急消息  → nats pub  '$mq9.AI.MAILBOX.MSG.{mail_address}.urgent'  '{...}'
订阅邮箱  → nats sub  '$mq9.AI.MAILBOX.MSG.{mail_address}.*'
公开发现  → nats req  '$mq9.AI.PUBLIC.LIST'              ''
```

任何 NATS 客户端直接接入，也可使用 RobustMQ SDK（Go/Python/Rust/JavaScript/Java/C#）或 `langchain-mq9` Toolkit。

---

## 核心特性

- 🤖 **mq9 — AI Agent 通信**：Agent 邮箱、优先级队列、公开服务发现，异步消息，发送方和接收方不需要同时在线
- 🦀 **Rust 构建**：无 GC，内存稳定可预测，无周期性内存波动，极小内存占用，从边缘设备到云端集群统一部署
- 🗄️ **统一存储层**：所有协议共享同一存储引擎，数据只写一份，按协议语义消费，无数据复制
- 🔌 **多协议原生支持**：MQTT 3.1/3.1.1/5.0、Kafka、NATS、AMQP、mq9 原生实现，各自保持完整协议语义
...
Phase 2（探索中）
  NATS 协议兼容 + AI Agent 通信（$mq9.AI.API.* 扩展）
