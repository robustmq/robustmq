...
| **Docker 镜像** | Docker 镜像 | `make release-docker` | 容器化部署 |
...
| 命令 | 功能 | 版本来源 | 说明 |
|------|------|----------|------|
| `make release-docker` | GHCR 应用镜像 | 自动读取 Cargo.toml | 构建并推送到 GitHub Container Registry（org=robustmq） |

> **版本说明**：`release-docker` 命令会自动从 `Cargo.toml` 读取版本号，并使用 `robustmq` 作为组织名称。
...
| `make release-docker` | 发布 Docker 镜像 | 自动读取 Cargo.toml | 构建并推送 Docker 镜像到 GHCR |
...
| **应用部署** | `make release-docker` | Docker 应用镜像 | 构建并推送应用镜像到 GitHub Container Registry（使用 cargo-chef 自动优化依赖缓存） |
