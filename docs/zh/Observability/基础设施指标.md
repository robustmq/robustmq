...
| 指标名称 | 类型 | 标签 | 描述 |
|---------|------|------|------|
| `mqtt_connector_messages_sent_success` | Counter | `connector_type`, `connector_name` | 指定 Connector 成功发送的消息数 |
| `mqtt_connector_messages_sent_failure` | Counter | `connector_type`, `connector_name` | 指定 Connector 发送失败的消息数 |
| `mqtt_connector_send_duration_ms` | Histogram | `connector_type`, `connector_name` | 指定 Connector 发送消息耗时分布（毫秒） |
| `mqtt_connector_retry_total` | Counter | `connector_type`, `connector_name`, `strategy` | Connector 重试总次数 |
| `mqtt_connector_messages_discarded_total` | Counter | `connector_type`, `connector_name`, `strategy` | Connector 按终态策略丢弃的消息数 |
| `mqtt_connector_dlq_messages_total` | Counter | `connector_type`, `connector_name`, `result` | Connector 写入死信队列的消息数（按结果 success/failure 统计） |
| `mqtt_connector_offset_commit_failure_total` | Counter | `connector_type`, `connector_name` | Connector offset 提交失败总次数 |
| `mqtt_connector_source_read_failure_total` | Counter | `connector_type`, `connector_name` | Connector 源 Topic 读取失败总次数 |
| `mqtt_connector_up` | Gauge | `connector_type`, `connector_name` | Connector 线程状态（1=运行中，0=已停止） |

### 聚合维度

| 指标名称 | 类型 | 标签 | 描述 |
|---------|------|------|------|
| `mqtt_connector_messages_sent_success_agg` | Counter | — | 所有 Connector 成功发送的消息总数 |
| `mqtt_connector_messages_sent_failure_agg` | Counter | — | 所有 Connector 发送失败的消息总数 |
