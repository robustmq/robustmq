...
$mq9.AI
  └── MAILBOX
        ├── CREATE                                   # 创建邮箱
        ├── MSG.{mail_id}                            # 发送/订阅消息（normal，默认，无后缀）
        ├── MSG.{mail_id}.urgent                     # 发送/订阅紧急消息
        ├── MSG.{mail_id}.critical                   # 发送/订阅最高优先级消息
        ├── MSG.{mail_id}.*                          # 订阅所有优先级
        ├── LIST.{mail_id}                           # 列出邮箱消息元数据
        └── DELETE.{mail_id}.{msg_id}                # 删除指定消息
```

| Subject | NATS 原语 | 说明 |
|---------|---------|------|
| `$mq9.AI.MAILBOX.CREATE` | request | 创建私有或公开邮箱 |
| `$mq9.AI.MAILBOX.MSG.{mail_id}` | pub/sub | 发送/订阅 normal 优先级消息（默认，无后缀） |
| `$mq9.AI.MAILBOX.MSG.{mail_id}.urgent` | pub/sub | 发送/订阅 urgent 优先级消息 |
| `$mq9.AI.MAILBOX.MSG.{mail_id}.critical` | pub/sub | 发送/订阅 critical 优先级消息 |
| `$mq9.AI.MAILBOX.MSG.{mail_id}.*` | sub | 订阅**所有**优先级（critical + urgent + normal） |
| `$mq9.AI.MAILBOX.LIST.{mail_id}` | request | 列出邮箱中所有消息的元数据 |
| `$mq9.AI.MAILBOX.DELETE.{mail_id}.{msg_id}` | request | 删除邮箱中的指定消息 |
| `$mq9.AI.PUBLIC.LIST` | sub | 发现所有公开邮箱，系统内置 |

### mail_id 格式

私有邮箱的 `mail_id` 由服务端生成，格式为 `mail-{nanoid}-{nanoid}`，例如：

```text
mail-d7a5072lko83gp7amga0-d7a5072lko83gp7amgag
```

公开邮箱的 `mail_id` 由用户在创建时通过 `name` 字段指定，支持点号分隔，例如 `task.queue`、`vision.results`。

`mail_id` 不可猜测即安全边界：知道 `mail_id` 即可发消息和订阅，不知道则无从操作，无 token，无 ACL。

---

## 3. 基础概念

**邮箱（Mailbox）** — mq9 的基本通信地址。通过 `MAILBOX.CREATE` 创建，返回 `mail_id`。不绑定 Agent 身份，一个 Agent 可以为不同任务创建不同邮箱。
...
| 优先级 | Subject 形式 | 典型场景 |
|--------|------------|---------|
| `critical` | `MSG.{mail_id}.critical` | 中止信号、紧急指令、安全事件 |
| `urgent` | `MSG.{mail_id}.urgent` | 任务中断、时效性指令 |
| `normal`（默认） | `MSG.{mail_id}`（无后缀） | 任务分发、结果返回、常规通信 |
...
| `name` | string | 公开邮箱必填 | — | 公开邮箱的自定义 `mail_id`，支持点号分隔（如 `task.queue`） |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `mail_id` | string | 邮箱唯一标识。私有邮箱为 `mail-{nanoid}-{nanoid}` 格式，公开邮箱为请求中的 `name` 值 |
...
Subject：`$mq9.AI.MAILBOX.MSG.{mail_id}[.{priority}]` · NATS 原语：publish

知道 `mail_id` 即可发送，无需授权。消息写入存储后立即返回，发送方不等待订阅者在线。

#### MSG-pub Subject 编码规则

| 优先级 | Subject |
|--------|---------|
| normal（默认） | `$mq9.AI.MAILBOX.MSG.{mail_id}`（无后缀） |
| urgent | `$mq9.AI.MAILBOX.MSG.{mail_id}.urgent` |
| critical | `$mq9.AI.MAILBOX.MSG.{mail_id}.critical` |

> 禁止通配符发布。`$mq9.AI.MAILBOX.MSG.*.*` 等通配符 subject 会被服务端拒绝，发布时必须指定精确的 `mail_id`。
...
Subject：`$mq9.AI.MAILBOX.MSG.{mail_id}[.{priority}|.*]` · NATS 原语：subscribe

#### MSG-sub 订阅模式

| Subject | 含义 |
|---------|------|
| `$mq9.AI.MAILBOX.MSG.{mail_id}.*` | 订阅**所有**优先级（critical + urgent + normal） |
| `$mq9.AI.MAILBOX.MSG.{mail_id}.critical` | 仅订阅 critical 优先级 |
| `$mq9.AI.MAILBOX.MSG.{mail_id}.urgent` | 仅订阅 urgent 优先级 |
| `$mq9.AI.MAILBOX.MSG.{mail_id}` | 仅订阅 normal 优先级（无后缀） |
...
Subject：`$mq9.AI.MAILBOX.LIST.{mail_id}` · NATS 原语：request/reply
...
Subject：`$mq9.AI.MAILBOX.DELETE.{mail_id}.{msg_id}` · NATS 原语：request/reply
...
`mail_id` 和 `msg_id` 编码在 subject 中，解析规则：倒数第一个 `.` 之后的 token 为 `msg_id`，之前的所有内容为 `mail_id`。例如 `task.queue.1003` 解析为 `mail_id=task.queue`，`msg_id=1003`。
...
```json
{"event":"created","mail_id":"task.queue","desc":"共享 Worker 任务队列","ttl":86400}
{"event":"created","mail_id":"vision.results","desc":"视觉处理结果","ttl":3600}
...
| 消费者状态 | 无 | 无（设计如此） |
| 消息顺序 | 不保证跨 subject | 同 mail_id 内按优先级排序，同优先级 FIFO |
