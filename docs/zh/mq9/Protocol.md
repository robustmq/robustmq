...
| `$mq9.AI.MAILBOX.{mail_id}.high` | pub/sub | 高优先级消息 |
| `$mq9.AI.MAILBOX.{mail_id}.normal` | pub/sub | 普通消息 |
| `$mq9.AI.MAILBOX.{mail_id}.low` | pub/sub | 低优先级消息 |
| `$mq9.AI.MAILBOX.{mail_id}.*` | sub | 订阅邮箱所有优先级 |
...
```bash
nats pub '$mq9.AI.MAILBOX.m-uuid-001.high'   'payload'  # 紧急，优先处理
nats pub '$mq9.AI.MAILBOX.m-uuid-001.normal' 'payload'  # 常规通信
nats pub '$mq9.AI.MAILBOX.m-uuid-001.low'    'payload'  # 后台，不紧急
```
...
存储层保证优先级顺序。边缘设备离线后上线，先收到 high 消息，再收到 normal，最后是 low。消费方无需自行排序。
...
```bash
nats sub '$mq9.AI.MAILBOX.m-uuid-001.*'     # 所有优先级
nats sub '$mq9.AI.MAILBOX.m-uuid-001.high'  # 仅高优先级
```
...
| `$mq9.AI.MAILBOX.{id}.low` | PUB/SUB | 否 | 低优先级消息 |
