...
# 最高优先级重配置
nats pub '$mq9.AI.MAILBOX.MSG.edge-agent.critical' '{
  "cmd": "reconfigure",
  "params": {"sampling_rate": 100}
}'

# 默认优先级（normal）例行任务
nats pub '$mq9.AI.MAILBOX.MSG.edge-agent' '{
  "cmd": "run_diagnostic",
  "target": "sensor-bank-2"
}'

# 边缘 Agent：重连后订阅——按优先级顺序收到所有存储的指令
nats sub '$mq9.AI.MAILBOX.MSG.edge-agent.*'
```
...
nats pub '$mq9.AI.MAILBOX.MSG.reply-a1b2c3' '{
  "result": "Bonjour le monde"
}'

# Agent A：准备好时订阅回复邮箱——结果已存储在那里
nats sub '$mq9.AI.MAILBOX.MSG.reply-a1b2c3.*'
```
...
nats pub '$mq9.AI.MAILBOX.MSG.agent.code-review' '{
  "file": "src/main.rs",
  "context": "性能审查"
}'
```
