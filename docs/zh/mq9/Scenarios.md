...
# 响应: {"mail_address": "d7a5072lko83@mq9"}

# 通过带外方式（如任务载荷）将 mail_address 传给子 Agent
nats pub '$mq9.AI.MAILBOX.MSG.task-dispatch@mq9' \
  '{"task": "summarize /data/corpus", "reply_to": "d7a5072lko83@mq9"}'
...
nats req '$mq9.AI.MAILBOX.CREATE' '{
  "ttl": 30,
  "public": true,
  "name": "worker.health.worker-42@mq9",
  "desc": "Worker 42 心跳"
}'
...
# 最高优先级重配置
nats pub '$mq9.AI.MAILBOX.MSG.edge-agent@mq9.critical' '{
  "cmd": "reconfigure",
  "params": {"sampling_rate": 100}
}'

# 默认优先级（normal）例行任务
nats pub '$mq9.AI.MAILBOX.MSG.edge-agent@mq9' '{
  "cmd": "run_diagnostic",
  "target": "sensor-bank-2"
}'

# 边缘 Agent：重连后订阅——按优先级顺序收到所有存储的指令
nats sub '$mq9.AI.MAILBOX.MSG.edge-agent@mq9.*'
```
...
# 响应: {"mail_address": "reply-a1b2c3@mq9"}
...
nats pub '$mq9.AI.MAILBOX.MSG.agent-b@mq9' '{
  ...
  "text": "Hello world",
  "lang": "fr",
  "reply_to": "reply-a1b2c3@mq9"
}'

# Agent A：继续其他工作...

# Agent B：处理请求后将结果发送到回复邮箱
nats pub '$mq9.AI.MAILBOX.MSG.reply-a1b2c3@mq9' '{
  "result": "Bonjour le monde"
}'

# Agent A：准备好时订阅回复邮箱——结果已存储在那里
nats sub '$mq9.AI.MAILBOX.MSG.reply-a1b2c3@mq9.*'
```
...
nats req '$mq9.AI.MAILBOX.CREATE' '{
  "ttl": 3600,
  "public": true,
  "name": "agent.code-review@mq9",
  "desc": "接受代码审查请求；以 JSON 格式返回发现结果"
}'

# 另一个 Agent：订阅 PUBLIC.LIST 发现可用能力
nats sub '$mq9.AI.PUBLIC.LIST'
# 收到条目如: {"name": "agent.code-review@mq9", "desc": "...", "mail_address": "agent.code-review@mq9"}

# 消费方 Agent：直接向发现的能力发送任务
nats pub '$mq9.AI.MAILBOX.MSG.agent.code-review@mq9' '{
  "file": "src/main.rs",
  "context": "性能审查"
}'
```
