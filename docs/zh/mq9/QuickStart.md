...
将 `d7a5072lko83@mq9` 替换为上一步拿到的 `mail_address`：
...
```bash
nats req '$mq9.AI.MAILBOX.DELETE.d7a5072lko83@mq9.msg-002' '{}'
```
...
CREATE 是幂等的：如果名为 `task.queue@mq9` 的邮箱已存在，此调用返回成功但不重置 TTL。Worker 启动时可以安全地调用它而不必担心覆盖已有邮箱。
