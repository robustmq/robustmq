...
```bash
nats pub '$mq9.AI.MAILBOX.MSG.d7a5072lko83'          'payload'  # 常规（默认，无后缀）
nats pub '$mq9.AI.MAILBOX.MSG.d7a5072lko83.urgent'   'payload'  # 紧急
nats pub '$mq9.AI.MAILBOX.MSG.d7a5072lko83.critical' 'payload'  # 最高优先级，优先处理
```
...
```bash
nats sub '$mq9.AI.MAILBOX.MSG.d7a5072lko83'           # 仅常规（默认）优先级
nats sub '$mq9.AI.MAILBOX.MSG.d7a5072lko83.*'         # 所有优先级（critical + urgent + normal）
nats sub '$mq9.AI.MAILBOX.MSG.d7a5072lko83.critical'  # 仅最高优先级
```
...
**第二，NATS 的 subject 模型天然适合邮箱寻址。** `$mq9.AI.MAILBOX.MSG.d7a5072lko83.critical`——一行字符串，清晰表达：协议、场景、消息类型、邮箱地址、优先级。裸地址（无后缀）承载常规优先级消息，通配符让 Agent 灵活选择订阅粒度。
...
```bash
nats pub '$mq9.AI.MAILBOX.MSG.orchestrator' '{"task_id":"t-001","result":"done"}'
```
...
```bash
nats pub '$mq9.AI.MAILBOX.MSG.device-001.critical' '{"cmd":"abort"}'
nats pub '$mq9.AI.MAILBOX.MSG.device-001'          '{"cmd":"update_config"}'
```
...
nats pub '$mq9.AI.MAILBOX.MSG.agent-b' '{"reply_to":"agent-a","payload":"..."}'
# B 上线后处理，结果发回 A 的邮箱
nats pub '$mq9.AI.MAILBOX.MSG.agent-a' '{"correlation_id":"...","result":"..."}'
```
