...
**第二，NATS 的 subject 模型天然适合邮箱寻址。** `$mq9.AI.MAILBOX.m-uuid-001.high`——一行字符串，清晰表达：协议、场景、邮箱地址、优先级。通配符让 Agent 灵活选择订阅粒度。
...
```bash
nats pub '$mq9.AI.MAILBOX.m-orchestrator.normal' '{"task_id":"t-001","result":"done"}'
```
...
```bash
nats pub '$mq9.AI.MAILBOX.m-device-001.high'   '{"cmd":"abort"}'
nats pub '$mq9.AI.MAILBOX.m-device-001.normal' '{"cmd":"update_config"}'
```
...
nats pub '$mq9.AI.MAILBOX.m-uuid-b.normal' '{"reply_to":"m-uuid-a","payload":"..."}'
# B 上线后处理，结果发回 A 的邮箱
nats pub '$mq9.AI.MAILBOX.m-uuid-a.normal' '{"correlation_id":"...","result":"..."}'
```
...
nats pub '$mq9.AI.MAILBOX.alert.anomaly.high' '{"type":"anomaly","confidence":0.95}'
```
