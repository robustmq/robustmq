...
邮箱是 Agent 的通信地址。临时的，TTL 驱动生命周期，到期自动销毁。Agent 为每个任务申请一个邮箱，拿到一个 mail_address，这就是它在这个任务里的通信地址。任务结束，邮箱自动过期清理。不需要注销，不需要手动删除。

**mail_address 不可猜测即安全边界。** mail_address 是系统生成的不可猜测字符串。知道 mail_address 就能往里发消息、能订阅——就像知道邮件地址就能发邮件。不知道 mail_address 就无从操作。没有 token，没有 ACL，没有额外的权限控制。
...
公开邮箱的 mail_address 本身就是地址，起一个有意义的名字——`task.queue`、`analytics.result`——比 UUID 更容易被其他 Agent 发现和理解。
...
nats req '$mq9.AI.MAILBOX.CREATE' '{"ttl":3600, "public":true, "name":"task.queue@mq9", "desc":"任务队列"}'
...
知道 mail_address 即可发送，无需任何授权。消息按优先级发送：

```bash
nats pub '$mq9.AI.MAILBOX.MSG.d7a5072lko83@mq9'          'payload'  # 常规（默认，无后缀）
nats pub '$mq9.AI.MAILBOX.MSG.d7a5072lko83@mq9.urgent'   'payload'  # 紧急
nats pub '$mq9.AI.MAILBOX.MSG.d7a5072lko83@mq9.critical' 'payload'  # 最高优先级，优先处理
```
...
```bash
nats sub '$mq9.AI.MAILBOX.MSG.d7a5072lko83@mq9'           # 仅常规（默认）优先级
nats sub '$mq9.AI.MAILBOX.MSG.d7a5072lko83@mq9.*'         # 所有优先级（critical + urgent + normal）
nats sub '$mq9.AI.MAILBOX.MSG.d7a5072lko83@mq9.critical'  # 仅最高优先级
```
...
**第二，NATS 的 subject 模型天然适合邮箱寻址。** `$mq9.AI.MAILBOX.MSG.d7a5072lko83@mq9.critical`——一行字符串，清晰表达：协议、场景、消息类型、邮箱地址、优先级。裸地址（无后缀）承载常规优先级消息，通配符让 Agent 灵活选择订阅粒度。
...
```bash
nats pub '$mq9.AI.MAILBOX.MSG.orchestrator@mq9' '{"task_id":"t-001","result":"done"}'
```
...
```bash
nats pub '$mq9.AI.MAILBOX.MSG.device-001@mq9.critical' '{"cmd":"abort"}'
nats pub '$mq9.AI.MAILBOX.MSG.device-001@mq9'          '{"cmd":"update_config"}'
```
...
nats pub '$mq9.AI.MAILBOX.MSG.agent-b@mq9' '{"reply_to":"agent-a@mq9","payload":"..."}'
# B 上线后处理，结果发回 A 的邮箱
nats pub '$mq9.AI.MAILBOX.MSG.agent-a@mq9' '{"correlation_id":"...","result":"..."}'
```
...
```bash
nats req '$mq9.AI.MAILBOX.CREATE' '{"ttl":3600,"public":true,"name":"alert.anomaly@mq9"}'
nats pub '$mq9.AI.MAILBOX.MSG.alert.anomaly@mq9.critical' '{"type":"anomaly","confidence":0.95}'
```
