...
async def discover_agents():
    async with Client(BROKER) as client:
        ...
        async def on_agent_registered(msg):
            ...
            # mail_address 就是 Agent 的 mq9 地址，直接用来发消息
...
- **mail_address**：发布者投 AgentCard 时，消息天然携带发布者的 mail_address，收到方直接就知道对方的 mq9 地址
...
async def send_task_via_mq9(sender_mail_address: str, target_mail_address: str, task: dict):
    async with Client(BROKER) as client:
        task["reply_to"] = sender_mail_address  # 告知对方结果发回哪里
        await client.send(
            target_mail_address,
