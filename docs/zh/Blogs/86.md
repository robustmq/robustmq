    ...
    async def on_message(msg):
        ...
        agent_b.invoke({
            "input": f"执行 {task['type']} 任务，数据是 {task['payload']}，"
                     f"把结果用 json.dumps 序列化后投入 mail_address 为 {task['reply_to']} 的 Mailbox，优先级 normal"
        })
...
async def main():
    ...
    agent_a.invoke({
        "input": "向 mail_address 为 agent-b-inbox 的 Mailbox 发送 normal 优先级消息：" + json.dumps(a2a_task, ensure_ascii=False)
    })
    ...
    # Step 6: 异步取结果，反序列化
    agent_a.invoke({
        "input": f"读取 mail_address 为 {result_mailbox} 的 Mailbox 中的消息，用 json.loads 解析后输出分析结果"
    })
...
**mail_address 需要带外传递**：Agent B 的 Mailbox 地址需要提前约定。如何让 Agent 自动发现彼此，是另一个问题——mq9 通过 `$SYSTEM.PUBLIC` 解决了这个问题，详见《用 mq9 给 A2A 补上 Agent 发现能力》。
