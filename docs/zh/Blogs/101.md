...
RobustMQ broker 已经在 58080 端口暴露 HTTP 服务（admin、management 等）。MCP 端点挂在这个 HTTP server 下的某个路径（如 `/mcp`），不开新端口、不做独立进程、不做独立项目。用户启动 broker 就有 MCP，零配置。
...
mq9 MCP 是 RobustMQ broker 的 HTTP 端点（58080 端口）下的一个路径。整体结构：

![img](../../images/101-1.jpg)

broker 启动时已经在 58080 端口跑了 HTTP 服务（admin、metrics、health 等）。MCP 端点（`/mcp`）作为这个 HTTP 服务的一个路径暴露出来，复用同一个 HTTP 端口。

AI Agent 通过 `http://broker:58080/mcp` 连接。普通客户端通过原生协议端口连接（mq9 走 4332、MQTT 走 1883 等）。两条路径并行。
...
**零配置。** MCP 端点是 broker 内置的，不需要任何配置。启动 broker 就有，端口跟着 58080 HTTP 服务。
...
**1. 它是 RobustMQ broker HTTP 端点的一部分，不是独立项目。** 不做独立 pip 包、不做独立 sidecar 进程、不做独立端口。挂在 broker 已有的 58080 HTTP 端口下的 `/mcp` 路径。
