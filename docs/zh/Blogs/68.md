...
私有邮箱的 mail_address 由系统生成，不可猜测。公开邮箱的 mail_address 由用户自定义，有意义的名字——`task.queue`、`analytics.result`。
...
| `$mq9.AI.MAILBOX.MSG.{mail_address}` | PUB/SUB | 是 | 发消息/接收消息，normal 优先级 |
| `$mq9.AI.MAILBOX.MSG.{mail_address}.urgent` | PUB/SUB | 是 | 紧急消息 |
| `$mq9.AI.MAILBOX.MSG.{mail_address}.critical` | PUB/SUB | 是 | 最高优先级消息 |
| `$mq9.AI.MAILBOX.LIST.{mail_address}` | PUB | — | 查看邮箱当前内容 |
| `$mq9.AI.MAILBOX.DELETE.{mail_address}.{msg_id}` | PUB | — | 删除指定消息 |

---

## $mq9.AI.MAILBOX.CREATE

创建邮箱。私有邮箱 mail_address 系统生成，公开邮箱 mail_address 用户自定义。
...
`public: true` 的邮箱创建后 mail_address 对外可见，TTL 到期自动移除。
...
nats pub '$mq9.AI.MAILBOX.MSG.{mail_address}' 'payload'

# 需要优先处理时，显式指定
nats pub '$mq9.AI.MAILBOX.MSG.{mail_address}.urgent'   'payload'
nats pub '$mq9.AI.MAILBOX.MSG.{mail_address}.critical' 'payload'
```
...
# 竞争消费
nats sub '$mq9.AI.MAILBOX.MSG.{mail_address}.>' --queue workers
```
...
```bash
nats request '$mq9.AI.MAILBOX.LIST.{mail_address}' ''
...
```bash
nats pub '$mq9.AI.MAILBOX.DELETE.{mail_address}.{msg_id}' ''
