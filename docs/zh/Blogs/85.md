...
# 创建 Mailbox，将 mail_address 告知其他 Agent，发送数据，立即返回
data_agent.invoke({
    "input": """
    1. 创建一个 Mailbox（私有或公开均可），TTL 设为 3600 秒，记录返回的 mail_address
...
创建一个 Mailbox，把 mail_address 告知多个 Agent，任意数量的 Agent 都可以订阅它，发送方只需往这个 Mailbox 发消息。

```python
# DataAgent：创建 Mailbox，发送消息
data_agent.invoke({
    "input": """
    创建一个 Mailbox，TTL 3600 秒，记录返回的 mail_address。
...
report_agent.invoke({
    "input": "订阅 mail_address 为 {mail_address} 的 Mailbox，收到消息后生成报告。"
})

# MonitorAgent：同样订阅这个 Mailbox
monitor_agent.invoke({
    "input": "订阅 mail_address 为 {mail_address} 的 Mailbox，收到消息后更新监控面板。"
})
```
...
async def main():
    ...
    await data_agent.ainvoke({"input": f"向 mail_address 为 {mail_address} 的 Mailbox 发送两条消息：第一条优先级 critical，内容 'URGENT: anomaly detected in dataset_v1'；第二条优先级 normal，内容 'dataset_v2: [record_1, record_2, record_3]'"})
    ...
    await analysis_agent.ainvoke({"input": f"读取 mail_address 为 {mail_address} 的 Mailbox 中的所有消息，按优先级顺序输出每条消息内容并完成分析。"})
