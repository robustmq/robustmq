...
```bash
nats pub '$mq9.AI.MAILBOX.m-uuid-001'          'payload'  # Normal (default, no suffix)
nats pub '$mq9.AI.MAILBOX.m-uuid-001.urgent'   'payload'  # Urgent
nats pub '$mq9.AI.MAILBOX.m-uuid-001.critical' 'payload'  # Critical, processed first
```
...
```bash
nats sub '$mq9.AI.MAILBOX.m-uuid-001'           # Normal (default) only
nats sub '$mq9.AI.MAILBOX.m-uuid-001.*'         # urgent + critical only (not bare normal)
...
**Second, NATS's subject model is naturally suited for mailbox addressing.** `$mq9.AI.MAILBOX.m-uuid-001.critical` — one string, four layers of information: mq9 protocol, AI scenario, mailbox address, priority. The bare subject (no suffix) carries normal-priority messages; wildcard subscriptions let Agents flexibly choose their granularity of interest.
