...
mq9's approach: **communication addresses are bound to tasks, not to Agent identity.** An Agent applies for a mailbox for each task, receives a mail_address, and that is its communication address for this task. When the task ends, the mailbox expires and is cleaned up automatically. No deregistration, no manual deletion, no residual state to clean up.
...
**mail_address unguessability is the security boundary.** mail_address is a system-generated unguessable string. Knowing the mail_address lets you send messages and subscribe — just like knowing an email address lets you send email. Without it, you can't interact with the mailbox. No token, no ACL.
...
| Discoverability | Private — only Agents who know the mail_address | Auto-registered to PUBLIC.LIST, discoverable by anyone |
| Use cases | Point-to-point messaging, task result delivery | Task queues, public channels, capability announcements |

A public mailbox's mail_address is its address — choose a meaningful name like `task.queue@mq9` or `analytics.result@mq9` rather than a UUID, making it easier for other Agents to discover and understand.
...
Knowing the mail_address is all that's needed — no authorization required. Messages are sent by priority:

```bash
nats pub '$mq9.AI.MAILBOX.MSG.d7a5072l@mq9'          'payload'  # Normal (default, no suffix)
nats pub '$mq9.AI.MAILBOX.MSG.d7a5072l@mq9.urgent'   'payload'  # Urgent
nats pub '$mq9.AI.MAILBOX.MSG.d7a5072l@mq9.critical' 'payload'  # Critical, processed first
```
...
```bash
nats sub '$mq9.AI.MAILBOX.MSG.d7a5072l@mq9'           # Normal (default) only
nats sub '$mq9.AI.MAILBOX.MSG.d7a5072l@mq9.*'         # all priorities (critical + urgent + normal)
nats sub '$mq9.AI.MAILBOX.MSG.d7a5072l@mq9.critical'  # Critical only
```
...
**Second, NATS's subject model is naturally suited for mailbox addressing.** `$mq9.AI.MAILBOX.MSG.d7a5072l@mq9.critical` — one string, five layers of information: mq9 protocol, AI scenario, message type, mailbox address, priority. The bare subject (no suffix) carries normal-priority messages; wildcard subscriptions let Agents flexibly choose their granularity of interest.
