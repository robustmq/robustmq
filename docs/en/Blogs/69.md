...
A public mailbox's mail_address is its address — choose a meaningful name like `task.queue` or `analytics.result` rather than a UUID, making it easier for other Agents to discover and understand.
...
```bash
nats pub '$mq9.AI.MAILBOX.MSG.d7a5072l'          'payload'  # Normal (default, no suffix)
nats pub '$mq9.AI.MAILBOX.MSG.d7a5072l.urgent'   'payload'  # Urgent
nats pub '$mq9.AI.MAILBOX.MSG.d7a5072l.critical' 'payload'  # Critical, processed first
```
...
```bash
nats sub '$mq9.AI.MAILBOX.MSG.d7a5072l'           # Normal (default) only
nats sub '$mq9.AI.MAILBOX.MSG.d7a5072l.*'         # all priorities (critical + urgent + normal)
nats sub '$mq9.AI.MAILBOX.MSG.d7a5072l.critical'  # Critical only
```
...
**Second, NATS's subject model is naturally suited for mailbox addressing.** `$mq9.AI.MAILBOX.MSG.d7a5072l.critical` — one string, five layers of information: mq9 protocol, AI scenario, message type, mailbox address, priority. The bare subject (no suffix) carries normal-priority messages; wildcard subscriptions let Agents flexibly choose their granularity of interest.
