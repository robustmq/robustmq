...
What is a `mail_address`? It's a network address for an Agent. What is `$mq9.AI.BROADCAST.system.capability`? It's DNS for the Agent world. What this article describes, almost in passing, is the sketch of a foundational protocol for an Agent internet.
