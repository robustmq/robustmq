    ...
    async def on_message(msg):
        ...
        agent_b.invoke({
            "input": f"Execute a {task['type']} task on data: {task['payload']}. "
                     ...
                     f"with mail_address {task['reply_to']} at normal priority."
        })
...
async def main():
    ...
    agent_a.invoke({
        "input": "Send a normal-priority message to the Mailbox with mail_address agent-b-inbox: " + json.dumps(a2a_task)
    })
    ...
    agent_a.invoke({
        "input": f"Read messages from the Mailbox with mail_address {result_mailbox}, parse with json.loads, and output the analysis result"
    })
...
**Out-of-band mail_address sharing**: Agent B's Mailbox address needs to be agreed on in advance. How agents automatically discover each other is a separate problem — mq9 solves it via `$SYSTEM.PUBLIC`, covered in *Adding Agent Discovery and Async Communication to A2A with mq9*.
