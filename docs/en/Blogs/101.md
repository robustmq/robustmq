...
RobustMQ broker already exposes HTTP service on port 58080 (admin, management, etc.). The MCP endpoint lives under a path of this HTTP server (e.g. `/mcp`). No new port, no new process, no new project. Starting the broker gives you MCP — zero configuration.
...
mq9 MCP is a path under the RobustMQ broker's HTTP endpoint on port 58080. Overall structure:

![img](../../images/101-1.jpg)

The broker already runs an HTTP server on port 58080 at startup (admin, metrics, health, etc.). The MCP endpoint (`/mcp`) is exposed as a path of this HTTP server, reusing the same port.

AI Agents connect via `http://broker:58080/mcp`. Regular clients connect via the native protocol ports (mq9 on 4332, MQTT on 1883, etc.). The two paths are parallel.
...
**Zero configuration.** The MCP endpoint is built into the broker; no configuration is needed. It is available the moment the broker starts, on the same port 58080 as the HTTP service.
...
**1. It is part of the broker's HTTP endpoint, not a standalone project.** No separate pip package, sidecar process, or separate port. Mounted at `/mcp` under the broker's existing port 58080 HTTP service.
