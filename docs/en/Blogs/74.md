...
# → {"mail_address":"abc123ef@mq9"}

# Sub-agent sends result (normal priority)
nats request '$mq9.AI.MAILBOX.MSG.abc123ef@mq9' '{"task_id":"t-001","result":"analysis complete"}'
# → {"msg_id":0}

# Orchestrator subscribes (including unread history)
nats subscribe '$mq9.AI.MAILBOX.MSG.abc123ef@mq9'

# Safety net: pull mailbox contents proactively
nats request '$mq9.AI.MAILBOX.LIST.abc123ef@mq9' ''
```
...
**mq9**: Each worker creates a public mailbox on startup with a well-known name (e.g., `status.worker001@mq9`), reports status periodically via `MAILBOX.MSG`, and sets a TTL for its mailbox lifetime. The orchestrator subscribes to each known worker's mailbox. When a worker stops updating, the TTL expiry removes its mailbox automatically — no explicit deregistration needed.

```bash
# Worker-001 starts, registers public status mailbox
nats request '$mq9.AI.MAILBOX.CREATE' '{"ttl":60,"public":true,"name":"status.worker001@mq9","desc":"worker status"}'

# Worker-001 reports status periodically (every 30s)
nats request '$mq9.AI.MAILBOX.MSG.status.worker001@mq9' '{"status":"running","load":0.3}'

# Orchestrator subscribes to each known worker's mailbox
nats subscribe '$mq9.AI.MAILBOX.MSG.status.worker001@mq9'
nats subscribe '$mq9.AI.MAILBOX.MSG.status.worker002@mq9'
```
...
**mq9**: Create a public task mailbox (e.g., `task.queue@mq9`); the orchestrator delivers tasks via `MAILBOX.MSG`; workers subscribe with a queue group to compete — each message goes to exactly one worker. Workers can join or leave dynamically with no configuration change.

```bash
# Orchestrator creates public task mailbox (one-time)
nats request '$mq9.AI.MAILBOX.CREATE' '{"public":true,"name":"task.queue@mq9","desc":"task queue"}'

# Orchestrator delivers tasks
nats request '$mq9.AI.MAILBOX.MSG.task.queue@mq9' '{"task_id":"t-001","type":"data_analysis"}'
nats request '$mq9.AI.MAILBOX.MSG.task.queue@mq9' '{"task_id":"t-002","type":"data_analysis"}'

# Workers compete using a queue group
nats subscribe '$mq9.AI.MAILBOX.MSG.task.queue@mq9' --queue task.workers
...
**mq9**: Create a public alert mailbox (e.g., `alert.payment@mq9`); the monitoring agent publishes alerts there; all interested agents subscribe independently. Messages are persisted — critical alerts are not missed on reconnect. The publisher does not need to know who is listening.

```bash
# Create public alert mailbox (one-time)
nats request '$mq9.AI.MAILBOX.CREATE' '{"public":true,"name":"alert.payment@mq9","desc":"payment domain alerts"}'

# Monitoring agent publishes critical alert
nats request '$mq9.AI.MAILBOX.MSG.alert.payment@mq9.critical' '{"level":"critical","detail":"payment timeout rate spiked to 15%"}'
...
nats request '$mq9.AI.MAILBOX.MSG.edge.device@mq9.critical' '{"command":"emergency_stop","reason":"temperature too high"}'
...
**mq9**: The approver has their own mail_address (a well-known public mailbox such as `approver.zhang@mq9`). The agent sends the approval request to `MAILBOX.MSG.{approver_mail_address}.urgent`; the approver's client subscribes to their own mailbox; after approval they reply via `MAILBOX.MSG.{agent_mail_address}`. Humans and agents use the exact same protocol — no separate approval service needed. A `correlation_id` in the payload ensures correct pairing.

```bash
# Approver's mailbox (pre-created public mailbox, name is the address)
nats request '$mq9.AI.MAILBOX.CREATE' '{"public":true,"name":"approver.zhang@mq9","desc":"approver mailbox"}'
...
**mq9**: Each agent creates a public mailbox with a well-known name on startup (e.g., `capability.worker001@mq9`) and posts a capability declaration. TTL controls registration validity — no renewal means automatic expiry. The orchestrator needing discovery creates a public query mailbox (e.g., `capability.query@mq9`), broadcasts its query there, and capable agents reply directly to the orchestrator's mailbox.

```bash
# Worker-001 registers capabilities (TTL=60, must renew periodically)
nats request '$mq9.AI.MAILBOX.CREATE' '{"public":true,"name":"capability.worker001@mq9","ttl":60}'
nats request '$mq9.AI.MAILBOX.MSG.capability.worker001@mq9' '{
  "capabilities":["data.analysis","report.generation"],
  "load":0.3,
  "reply_to":"capability.worker001@mq9"
}'
...
| Orchestrator monitors all worker status | O(n) polling | SCAN polling | Compacted topic; lag | Public mailbox (`status.worker001@mq9`) + TTL auto-expiry | High |
| Task fan-out, multiple workers compete | Build own scheduler | BRPOP — unreliable | Rebalance causes pause | Public mailbox (`task.queue@mq9`) + queue group; zero config | Medium |
| Agent detects anomaly, broadcasts alert | Maintain subscriber list | No persistence | Need multiple consumer groups | Public alert mailbox (`alert.payment@mq9`) + multi-subscriber | Medium |
| Cloud sends commands to offline edge agent | Immediate failure | No priority; high memory cost | No native priority | MSG three-level priority + persistence (`edge.device@mq9`) | High |
| Agent sends approval request; human approves and flow continues | At least three systems | Both sides polling | Approver needs Kafka client | Human and agent share one protocol, bidirectional MSG (`approver.zhang@mq9`) | High |
| Agent A asks Agent B a question; B may be offline | Immediate failure if offline | Two Lists + both sides polling | Two topics + filter responses | MSG + reply_to; offline-safe (`agent.a@mq9`, `agent.b@mq9`) | Medium |
| Agent registers capabilities; others discover and call | Needs registry | SCAN polling | Compacted topic; complex | Public mailbox (`capability.query@mq9`) + TTL auto-cleanup | High |
...
async def scenario2_global_status_awareness():
    ...
    await nc.subscribe("$mq9.AI.MAILBOX.MSG.status.worker001@mq9", cb=on_status)
    await nc.subscribe("$mq9.AI.MAILBOX.MSG.status.worker002@mq9", cb=on_status)

    # Worker-001 reports status
    await nc.request(
        "$mq9.AI.MAILBOX.MSG.status.worker001@mq9",
        json.dumps({"status": "running", "load": 0.3, "capabilities": ["data.analysis"]}).encode(),
        timeout=3
    )
    # Worker-002 reports status
    await nc.request(
        "$mq9.AI.MAILBOX.MSG.status.worker002@mq9",
...
async def scenario8_capability_discovery():
    ...
    await nc.request(
        "$mq9.AI.MAILBOX.CREATE",
        json.dumps({"public": True, "name": "capability.worker001@mq9", "ttl": 60}).encode(),
        timeout=3
    )
    await nc.request(
        "$mq9.AI.MAILBOX.MSG.capability.worker001@mq9",
