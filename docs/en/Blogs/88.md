...
async def discover_agents():
    async with Client(BROKER) as client:
        ...
        async def on_agent_registered(msg):
            ...
            # mail_address is the agent's mq9 address, use it directly to send messages
            mq9_addr = msg.mail_address
...
- **mail_address**: When a publisher posts an AgentCard, the message naturally carries the publisher's mail_address — the subscriber already knows the sender's mq9 address
...
```python
async def send_task_via_mq9(sender_mail_address: str, target_mail_address: str, task: dict):
    async with Client(BROKER) as client:
        task["reply_to"] = sender_mail_address  # tell the receiver where to send the result
        await client.send(
            target_mail_address,
