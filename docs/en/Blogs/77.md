...
```bash
nats pub '$mq9.AI.INBOX.worker001@mq9.normal' '{"type":"task","content":"Analyze Q3 data"}'
```
...
```bash
nats pub '$mq9.AI.INBOX.orchestrator@mq9.normal' '{"type":"task_result","result":"anomaly rate 2.3%"}'
```
...
```bash
nats pub '$mq9.AI.INBOX.approver.zhang@mq9.urgent' '{"type":"approval","content":"requesting access to external API"}'
```
...
```bash
nats pub '$mq9.AI.INBOX.edge.device@mq9.urgent' '{"command":"emergency_stop"}'
```
...
```bash
nats pub '$mq9.AI.BROADCAST.capability.query@mq9' '{"need":"data.analysis"}'
```
