...
# Create a Mailbox, share the mail_address with other agents, send data, return immediately
data_agent.invoke({
    "input": """
    1. Create a Mailbox (private or public), TTL 3600 seconds, record the returned mail_address
...
Create a Mailbox and share the mail_address with multiple agents. Any number of agents can subscribe to it. The sender just drops messages into the Mailbox without knowing who's listening.

```python
# DataAgent: create Mailbox and send message
data_agent.invoke({
    "input": """
    Create a Mailbox, TTL 3600 seconds, record the returned mail_address.
...
report_agent.invoke({
    "input": "Subscribe to the Mailbox with mail_address {mail_address}. Generate a report when a message arrives."
})

# MonitorAgent: also subscribe to this Mailbox
monitor_agent.invoke({
    "input": "Subscribe to the Mailbox with mail_address {mail_address}. Update the monitoring dashboard when a message arrives."
})
```
...
async def main():
    ...
    # 1. Create a Mailbox and get the mail_address
    mail_address = await CreateMailboxTool(server=SERVER)._arun(ttl=3600)
    ...
    await data_agent.ainvoke({"input": f"Send two messages to the Mailbox with mail_address {mail_address}: first with priority critical, content 'URGENT: anomaly detected in dataset_v1'; second with priority normal, content 'dataset_v2: [record_1, record_2, record_3]'"})
    ...
    await analysis_agent.ainvoke({"input": f"Read all messages from the Mailbox with mail_address {mail_address}, output each message in priority order, and complete the analysis."})
