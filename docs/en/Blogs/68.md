...
```bash
nats pub '$mq9.AI.MAILBOX.{mail_id}'          'payload'   # normal (default, no suffix)
nats pub '$mq9.AI.MAILBOX.{mail_id}.urgent'   'payload'   # urgent
nats pub '$mq9.AI.MAILBOX.{mail_id}.critical' 'payload'   # critical (highest)
```
...
```bash
nats sub '$mq9.AI.MAILBOX.{mail_id}'           # Normal (default) only
nats sub '$mq9.AI.MAILBOX.{mail_id}.*'         # urgent + critical only (not bare normal)
nats sub '$mq9.AI.MAILBOX.{mail_id}.critical'  # Critical only
```
...
| `$mq9.AI.MAILBOX.{id}` | PUB/SUB | Yes | Normal messages (default, no suffix) |
| `$mq9.AI.MAILBOX.{id}.urgent` | PUB/SUB | Yes | Urgent messages |
| `$mq9.AI.MAILBOX.{id}.critical` | PUB/SUB | Yes | Critical messages (highest priority) |
