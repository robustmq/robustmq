...
**TTL**: Declared at mailbox creation time; the mailbox is automatically destroyed on expiry and its messages are cleaned up along with it. CREATE is **not** idempotent — creating a mailbox that already exists returns an error (`mailbox xxx already exists`).
...
|-------|------|----------|-------------|
| `name` | string | No | The mail_address for the mailbox; must conform to the format specification. Auto-generated by broker if omitted. |
...
**Subject (priority encoded in suffix)**

| Subject | Priority |
|---------|----------|
| `$mq9.AI.MSG.SEND.{mail_address}` | normal (default) |
| `$mq9.AI.MSG.SEND.{mail_address}.urgent` | urgent |
| `$mq9.AI.MSG.SEND.{mail_address}.critical` | critical |
...
# Message-level TTL — expires 300 seconds after send
nats request '$mq9.AI.MSG.SEND.agent.translator.inbox' \
  -H "mq9-ttl:300" \
...
# Urgent message
nats request '$mq9.AI.MSG.SEND.agent.translator.inbox.urgent' '{"alert":"please expedite"}'
# Response
{"error":"","msg_id":4}
```
