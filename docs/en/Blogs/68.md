...
**mail_address**: The communication address returned when creating a mailbox via `MAILBOX.CREATE`. Not bound to Agent identity — one Agent can apply for different mail_addresss for different tasks. Leave it alone when done; TTL handles cleanup automatically.

Private mailbox mail_addresss are system-generated and unguessable. Public mailbox mail_addresss are user-defined with meaningful names — `task.queue`, `analytics.result`.

**mail_address unguessability is the security boundary.** Knowing the mail_address lets you send messages and subscribe. Without it, you can't interact with the mailbox. No token, no ACL.
...
| `MAILBOX` | `$mq9.AI.MAILBOX.MSG.{mail_address}.{priority}` | PUB | Send a message to a mailbox |
| `MAILBOX` | `$mq9.AI.MAILBOX.MSG.{mail_address}.*` | SUB | Subscribe to a mailbox; receive all non-expired messages in full |
...
Create a mailbox. Private mailbox mail_addresss are system-generated; public mailbox mail_addresss are user-defined.
...
## $mq9.AI.MAILBOX.MSG.{mail_address}.{priority}

### Sending Messages

Knowing the mail_address is all that's needed to send — no authorization required.

```bash
nats pub '$mq9.AI.MAILBOX.MSG.{mail_address}'          'payload'   # normal (default, no suffix)
nats pub '$mq9.AI.MAILBOX.MSG.{mail_address}.urgent'   'payload'   # urgent
nats pub '$mq9.AI.MAILBOX.MSG.{mail_address}.critical' 'payload'   # critical (highest)
```
...
| reply_to | Reply address; the recipient sends responses to this mail_address |
...
```bash
nats sub '$mq9.AI.MAILBOX.MSG.{mail_address}'           # Normal (default) only
nats sub '$mq9.AI.MAILBOX.MSG.{mail_address}.*'         # all priorities (critical + urgent + normal)
nats sub '$mq9.AI.MAILBOX.MSG.{mail_address}.critical'  # Critical only
```
...
```json
# Added
{"event": "created", "mail_address": "task.queue", "desc": "Task queue", "ttl": 3600}

# Removed
{"event": "expired", "mail_address": "task.queue"}
```
