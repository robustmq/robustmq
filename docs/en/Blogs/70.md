...
**mail_address unguessability is the security boundary.** mail_address is a system-generated unguessable string. Knowing the mail_address lets you send messages and subscribe; without it, you can't interact with the mailbox. No token, no ACL.
...
| `MAILBOX.MSG.{mail_address}.{priority}` | Send a message to a mailbox |
| `MAILBOX.MSG.{mail_address}.*` | Subscribe to a mailbox; receive all non-expired messages in full |
...
- **mail_address is not bound to Agent identity.** One Agent, multiple mailboxes; leave them alone when done; TTL handles cleanup.
...
- **Wildcard subscriptions across all mailboxes are prohibited.** Subscriptions must specify an exact mail_address; the broker enforces this.
