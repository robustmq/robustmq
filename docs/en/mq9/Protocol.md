...
$mq9.AI
  └── MAILBOX
        ...
        ├── MSG.{mail_id}                            # Send/subscribe (normal, default, no suffix)
        ├── MSG.{mail_id}.urgent                     # Send/subscribe urgent messages
        ├── MSG.{mail_id}.critical                   # Send/subscribe highest-priority messages
        ├── MSG.{mail_id}.*                          # Subscribe to all priorities
        ├── LIST.{mail_id}                           # List mailbox message metadata
        └── DELETE.{mail_id}.{msg_id}                # Delete a specific message
```

| Subject | NATS primitive | Description |
|---------|---------------|-------------|
| `$mq9.AI.MAILBOX.CREATE` | request | Create a private or public mailbox |
| `$mq9.AI.MAILBOX.MSG.{mail_id}` | pub/sub | Send/subscribe normal priority messages (default, no suffix) |
| `$mq9.AI.MAILBOX.MSG.{mail_id}.urgent` | pub/sub | Send/subscribe urgent priority messages |
| `$mq9.AI.MAILBOX.MSG.{mail_id}.critical` | pub/sub | Send/subscribe critical priority messages |
| `$mq9.AI.MAILBOX.MSG.{mail_id}.*` | sub | Subscribe to **all** priorities (critical + urgent + normal) |
| `$mq9.AI.MAILBOX.LIST.{mail_id}` | request | List metadata for all messages in a mailbox |
| `$mq9.AI.MAILBOX.DELETE.{mail_id}.{msg_id}` | request | Delete a specific message from a mailbox |
| `$mq9.AI.PUBLIC.LIST` | sub | Discover all public mailboxes, system-managed |

### mail_id Format

Private mailbox `mail_id` values are server-generated, in the format `mail-{nanoid}-{nanoid}`, for example:

```text
mail-d7a5072lko83gp7amga0-d7a5072lko83gp7amgag
```

Public mailbox `mail_id` values are user-defined via the `name` field at creation time. Dot-separated names are supported, e.g. `task.queue`, `vision.results`.

`mail_id` unguessability is the security boundary: knowing the `mail_id` lets you send and subscribe; without it, there is no way to interact with the mailbox. No token, no ACL.

---

## 3. Core Concepts

**Mailbox** — mq9's fundamental communication address. Created via `MAILBOX.CREATE`, which returns a `mail_id`. Not bound to Agent identity — one Agent can create different mailboxes for different tasks.
...
|----------|-------------|-------------|
| `critical` | `MSG.{mail_id}.critical` | Abort signals, emergency commands, security events |
| `urgent` | `MSG.{mail_id}.urgent` | Task interrupts, time-sensitive instructions |
| `normal` (default) | `MSG.{mail_id}` (no suffix) | Task dispatch, result delivery, routine communication |
...
| `name` | string | Required for public | — | Custom `mail_id` for a public mailbox; dot-separated names supported (e.g. `task.queue`) |
...
|-------|------|-------------|
| `mail_id` | string | Mailbox identifier. Private mailboxes use `mail-{nanoid}-{nanoid}` format; public mailboxes use the `name` value from the request |
...
```json
{"mail_id":"task.queue","is_new":true}
```
...
```json
{"mail_id":"task.queue","is_new":false}
```
...
Subject: `$mq9.AI.MAILBOX.MSG.{mail_id}[.{priority}]` · NATS primitive: publish

Knowing the `mail_id` is enough to send — no authorization required. Messages are written to storage and return immediately; the sender does not wait for subscribers to be online.

#### MSG-pub Subject Encoding

| Priority | Subject |
|----------|---------|
| normal (default) | `$mq9.AI.MAILBOX.MSG.{mail_id}` (no suffix) |
| urgent | `$mq9.AI.MAILBOX.MSG.{mail_id}.urgent` |
| critical | `$mq9.AI.MAILBOX.MSG.{mail_id}.critical` |

> Wildcard publishing is prohibited. Subjects like `$mq9.AI.MAILBOX.MSG.*.*` will be rejected by the server. Publishing requires an exact `mail_id`.
...
| `reply_to` | string | Reply address; the recipient sends their response to this `mail_id` |
...
Subject: `$mq9.AI.MAILBOX.MSG.{mail_id}[.{priority}|.*]` · NATS primitive: subscribe

#### MSG-sub Subscription Patterns

| Subject | Meaning |
|---------|---------|
| `$mq9.AI.MAILBOX.MSG.{mail_id}.*` | Subscribe to **all** priorities (critical + urgent + normal) |
| `$mq9.AI.MAILBOX.MSG.{mail_id}.critical` | critical only |
| `$mq9.AI.MAILBOX.MSG.{mail_id}.urgent` | urgent only |
| `$mq9.AI.MAILBOX.MSG.{mail_id}` | normal only (no suffix) |
...
Subject: `$mq9.AI.MAILBOX.LIST.{mail_id}` · NATS primitive: request/reply
...
| Field | Type | Description |
|-------|------|-------------|
| `mail_id` | string | Mailbox identifier |
...
Subject: `$mq9.AI.MAILBOX.DELETE.{mail_id}.{msg_id}` · NATS primitive: request/reply
...
The `mail_id` and `msg_id` are encoded in the subject. Parsing rule: the token after the last `.` is the `msg_id`; everything before it is the `mail_id`. For example, `task.queue.1003` parses as `mail_id=task.queue`, `msg_id=1003`.
...
Public mailbox (mail_id contains dots):
...
| `mail_id` | string | Public mailbox identifier |
...
| `mail_id` | string | Identifier of the expired mailbox |
...
```json
{"event":"created","mail_id":"task.queue","desc":"Shared worker task queue","ttl":86400}
{"event":"created","mail_id":"vision.results","desc":"Vision processing results","ttl":3600}
{"event":"expired","mail_id":"vision.results"}
```
...
| Message ordering | Not guaranteed across subjects | Within a mail_id: priority-ordered; within a priority: FIFO |
