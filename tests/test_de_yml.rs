...
#[test]
fn test_read_yml() {
    let f = std::fs::File::open("config/log4rs.yml").expect("Could not open file.");
   // let data: serde_yaml::Value = serde_yaml::from_str(f).unwrap();
    let data:serde_yaml::Value = serde_yaml::from_reader(f).expect("Could not read values.");
    println!("refresh_rate is : {}", data["refresh_rate"].as_str().unwrap());
}