    ...
    async fn topic_delete_integration_test() {
        ...
        let monitor_topic_resp: Value = admin_client
            .get_monitor_data(&MonitorDataReq {
                data_type: "total_number_of_topics".to_string(),
                topic_name: Some(topic.clone()),
                ..Default::default()
            })
            .await
            .expect("failed to query topic monitor data before delete");
        let topic_data_obj = monitor_topic_resp["data"]
            .as_object()
            .expect("topic monitor data should be a JSON object");
        assert!(
            !topic_data_obj.is_empty(),
            "topic monitor data should be non-empty before delete"
        );

        let monitor_sub_resp: Value = admin_client
            .get_monitor_data(&MonitorDataReq {
                data_type: "total_number_of_subscriptions".to_string(),
                topic_name: Some(topic.clone()),
                ..Default::default()
            })
            .await
            .expect("failed to query subscription monitor data before delete");
        let sub_data_obj = monitor_sub_resp["data"]
            .as_object()
            .expect("subscription monitor data should be a JSON object");
        assert!(
            !sub_data_obj.is_empty(),
        ...
        let data_obj = &monitor_topic_after["data"];
        assert!(
            data_obj.is_object() && data_obj.as_object().map_or(true, |o| o.is_empty()),
