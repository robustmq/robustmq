...
pub fn read(
    ...
    mut bytes: Bytes,
) -> Result<(PubRel, Option<PubRelProperties>), MQTTProtocolError> {
...
mod properties {
    ...
    pub fn read(bytes: &mut Bytes) -> Result<Option<PubRelProperties>, MQTTProtocolError> {
...
fn reason(num: u8) -> Result<PubRelReason, MQTTProtocolError> {
    let code = match num {
