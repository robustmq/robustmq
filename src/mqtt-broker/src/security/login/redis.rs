...
/// Redis authentication check entry function
pub async fn redis_check_login(
...
mod tests {
    ...
    async fn test_redis_plain_authentication() {
        ...
        let result = redis_auth.apply().await.unwrap();
        assert!(result);
    }
