...
pub async fn save_topic_retain_message(
    ...
    if publish.retain {
        ...
        if publish.payload.is_empty() {
            ...
            match topic_storage
                .set_retain_message(topic_name.clone(), retain_message.clone())
                .await
            {
                Ok(_) => {
                    cache_manager
...
mod tests {
    ...
    use crate::core::retain_message::save_retain_message;
    ...
    async fn send_retain_message_test() {
        ...
        match topic_storage
            .set_retain_message(topic.topic_id, retain_message.clone())
            .await
        {
            Ok(_) => {}
            Err(e) => {
