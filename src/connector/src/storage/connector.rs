...
impl ConnectorStorage {
    ...
    pub async fn list_connector(
        &self,
        connector_name: &str,
    ) -> Result<Vec<MQTTConnector>, CommonError> {
    ...
    pub async fn list_all_connectors(&self) -> Result<Vec<MQTTConnector>, CommonError> {
    ...
    pub async fn update_connector(&self, connector: MQTTConnector) -> Result<(), CommonError> {
        let config = broker_config();
