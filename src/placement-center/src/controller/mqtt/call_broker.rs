...
async fn start_call_thread(
    ...
    tokio::spawn(async move {
        ...
        if let Some(node_send) = call_manager.get_node_sender(&cluster_name, node.node_id) {
            ...
            info!("Call thread starts successfully, Inner communication between Placement Center and MQTT Broker node [{:?}].",node.node_id);
            loop {
                select! {
                    val = raw_stop_rx.recv() =>{
                        if let Ok(flag) = val {
                            if flag {
                                info!("Thread stops successfully, Inner communication between Placement Center and MQTT Broker node [{:?}].",node.node_id);
                                break;
                            }
                        }
                    },
