...
impl RedisAuthStorageAdapter {
    pub fn new(config: RedisConfig) -> Result<Self, CommonError> {
    ...
    fn query_user_ids(&self, conn: &mut RedisConnection) -> Result<Vec<String>, CommonError> {
        Self::exec_query_ids(
            conn,
    ...
    fn query_acl_ids(&self, conn: &mut RedisConnection) -> Result<Vec<String>, CommonError> {
        Self::exec_query_ids(conn, &self.config.query_acl, RedisAuthAcl::redis_acls_key())
    }

    fn query_blacklist_ids(&self, conn: &mut RedisConnection) -> Result<Vec<String>, CommonError> {
        Self::exec_query_ids(
            conn,
