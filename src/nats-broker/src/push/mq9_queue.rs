...
use crate::push::mq9_fanout::send_mq9_packet;
...
pub struct Mq9QueuePushManagerParams {
...
pub struct Mq9QueuePushManager {
    ...
    group_name: String,
    subject: String,
    consumer: Option<PriorityGroupConsumer>,
    round_robin: u64,
}

impl Mq9QueuePushManager {
    pub fn new(p: Mq9QueuePushManagerParams) -> Self {
        Mq9QueuePushManager {
    ...
    pub async fn start(&mut self, stop_sx: &broadcast::Sender<bool>) {
        ...
        let label = format!("MQ9 QueuePushManager[{}]", self.queue_key());
        loop {
            select! {
                val = stop_rx.recv() => {
    ...
    fn active_subscribers(&self, queue_key: &str) -> Option<Vec<NatsSubscriber>> {
        let bucket_mgr = self.subscribe_manager.mq9_queue_push.get(queue_key)?;
    ...
    async fn send_messages(&mut self) -> Result<usize, NatsBrokerError> {
        ...
        let base_tag = subject_message_tag(&self.tenant, &self.subject);
        let records = consumer
            .next_messages_by_tags(&self.tenant, &self.subject, &base_tag, &read_config)
            .await
            .map_err(NatsBrokerError::from)?;

        if let Some(info) = self.subscribe_manager.mq9_queue_push_thread.get(&queue_key) {
            info.last_pull_time
                .lock()
        ...
        for record in &records {
            ...
            .await
            {
                ...
                Ok(false) => {
                    all_delivered = false;
                    warn!(
                        "MQ9 queue [{}]: no subscriber available, will retry",
                        queue_key
                    );
                }
                Err(e) => {
                    all_delivered = false;
                    warn!("MQ9 queue send error [{}]: {}", queue_key, e);
                }
            }
        }
...
async fn mq9_round_robin_send(
    ...
    for i in 0..subscribers.len() {
        ...
        if conf.broker_id == subscriber.broker_id {
            match send_mq9_packet(connection_manager, subscriber, record).await {
                Ok(true) => return Ok(true),
                Ok(false) => {}
                Err(NatsBrokerError::ConnectionNotFound(_)) => {
                    warn!(
                        "MQ9 queue subscriber gone: connect_id={} sid={}",
                ...
                Err(e) => {
                    debug!(
                        "MQ9 queue send failed [connect_id={}, sid={}]: {}",
            ...
            .await
            {
                ...
                Err(e) => {
                    warn!(
                        "MQ9 queue remote send failed [broker_id={}, connect_id={}, sid={}]: {}",
