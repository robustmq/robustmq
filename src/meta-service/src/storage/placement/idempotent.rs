...
use rocksdb_engine::storage::meta_metadata::{
    engine_delete_by_meta_metadata, engine_exists_by_meta_metadata, engine_save_by_meta_metadata,
};
...
impl IdempotentStorage {
    ...
    pub fn save(
        ...
        engine_save_by_meta_metadata(self.rocksdb_engine_handler.clone(), &key, seq_num)
    }
    ...
    pub fn exists(
        ...
        let key = key_resource_idempotent(cluster_name, producer_id, seq_num);
        engine_exists_by_meta_metadata(self.rocksdb_engine_handler.clone(), &key)
    }
}
