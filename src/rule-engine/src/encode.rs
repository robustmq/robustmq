...
pub enum DataEncodeType {
    ...
    /// Requires exactly one record; fails if there are zero or multiple.
    #[default]
    JsonObject,
    /// Serialize all records as a JSON array.
    JsonArray,
    /// Serialize each record as one line of JSON, newline-separated.
    JsonLines,
    /// Serialize records as CSV text; first record's keys become the header row.
    Csv,
    /// Extract the `message` field from the first record as plain UTF-8 text.
    PlainText,
    /// Extract the `bytes` array field from the first record back to raw bytes.
    Bytes,
}
...
pub fn operator_encode_data(
    ...
    match encode_type {
        ...
        DataEncodeType::JsonLines => encode_json_lines(records),
...
fn encode_json_lines(records: Vec<Map<String, Value>>) -> Result<Bytes, CommonError> {
...
mod tests {
    ...
    #[test]
    fn encode_json_object_ok() {
        let records = vec![make_record(&[("k", json!("v")), ("n", json!(1))])];
        let result = operator_encode_data(records, &DataEncodeType::JsonObject).unwrap();
    ...
    #[test]
    fn encode_json_object_fails_on_multiple() {
        let records = vec![
            make_record(&[("k", json!("a"))]),
            make_record(&[("k", json!("b"))]),
        ];
        let err = operator_encode_data(records, &DataEncodeType::JsonObject).unwrap_err();
        assert!(err.to_string().contains("exactly 1 record"));
    }

    #[test]
    fn encode_json_array_ok() {
        let records = vec![
            make_record(&[("k", json!("a"))]),
            make_record(&[("k", json!("b"))]),
        ];
        let result = operator_encode_data(records, &DataEncodeType::JsonArray).unwrap();
        let parsed: Value = serde_json::from_slice(&result).unwrap();
    ...
    #[test]
    fn encode_json_array_empty() {
        let result = operator_encode_data(vec![], &DataEncodeType::JsonArray).unwrap();
        assert_eq!(&result[..], b"[]");
    }

    #[test]
    fn encode_json_lines_ok() {
        let records = vec![
            make_record(&[("d", json!("d1"))]),
            make_record(&[("d", json!("d2"))]),
        ];
        let result = operator_encode_data(records, &DataEncodeType::JsonLines).unwrap();
    ...
    #[test]
    fn encode_csv_ok() {
        // CSV headers are built from all keys and sorted lexicographically.
        let records = vec![
            make_record(&[("ts", json!("2026-03-03")), ("temp", json!(36.5))]),
            make_record(&[("ts", json!("2026-03-04")), ("temp", json!(37.0))]),
        ];
        let result = operator_encode_data(records, &DataEncodeType::Csv).unwrap();
        let text = std::str::from_utf8(&result).unwrap();
        let lines: Vec<&str> = text.lines().collect();
        assert_eq!(lines[0], "temp,ts");
        assert_eq!(lines[1], "36.5,2026-03-03");
    }

    #[test]
    fn encode_csv_include_keys_from_all_records() {
        let records = vec![
    ...
    #[test]
    fn encode_plain_text_ok() {
        let records = vec![make_record(&[("message", json!("hello world"))])];
        let result = operator_encode_data(records, &DataEncodeType::PlainText).unwrap();
        assert_eq!(&result[..], b"hello world");
    }

    #[test]
    fn encode_plain_text_fails_on_multiple() {
        let records = vec![
            make_record(&[("message", json!("hello"))]),
            make_record(&[("message", json!("world"))]),
        ];
        let err = operator_encode_data(records, &DataEncodeType::PlainText).unwrap_err();
        assert!(err.to_string().contains("exactly 1 record"));
    }

    #[test]
    fn encode_raw_bytes_ok() {
        let records = vec![make_record(&[("bytes", json!([137, 80, 78, 71]))])];
        let result = operator_encode_data(records, &DataEncodeType::Bytes).unwrap();
        assert_eq!(&result[..], &[137u8, 80, 78, 71]);
    }

    #[test]
    fn encode_raw_bytes_fails_on_multiple() {
        let records = vec![
            make_record(&[("bytes", json!([1, 2]))]),
            make_record(&[("bytes", json!([3, 4]))]),
        ];
        let err = operator_encode_data(records, &DataEncodeType::Bytes).unwrap_err();
        assert!(err.to_string().contains("exactly 1 record"));
    }
}
