...
use raft_proto::eraftpb::{ConfChange, Snapshot, HardState};
...
impl MetaRaft {
    ...
    async fn on_ready(&self, raft_node: &mut RawNode<RaftRocksDBStorage>) {
        ...
        if *ready.snapshot() != Snapshot::default() {
            ...
            info_meta(&format!("save snapshot,term:{},index:{}",s.get_metadata().get_term(),s.get_metadata().get_index()));
        ...
        if !ready.persisted_messages().is_empty() {
            info_meta(&format!("save persisted_messages!!!,len:{:?}",ready.persisted_messages().len()));
    ...
    fn handle_committed_entries(&self, entrys: Vec<Entry>) {
        info_meta(&format!("handle committed entries !!!,len:{}",entrys.len()));
        for entry in entrys {
