...
pub async fn build_placement_center_client(
    addr: &String,
) -> Result<PlacementCenterServiceClient<Channel>, RobustMQError> {
    match PlacementCenterServiceClient::connect(format!("http://{}", addr)).await {
...
pub async fn send_raft_conf_change(
    ...
    let mut client: PlacementCenterServiceClient<Channel> =
        match PlacementCenterServiceClient::connect(format!("http://{}", addr)).await {
            Ok(client) => client,
            Err(err) => return Err(RobustMQError::TonicTransport(err)),
        };
